...
// Import eHandler langsung
import axlyHandler from '../axly.js'
...
async function LoadDataBase(axly, m) {
	try {
		...
		const defaultSetBot = {
			...
			privateonly: true,
			didyoumean: true,
			author: global.author || 'axly',
			authorPrefix: '',
			autobackup: false,
			botname: global.botname || 'axly Bot',
...
async function MessagesUpsert(axly, message, store) {
    try {
        ...
        setImmediate(async () => {
            try {
                await axlyHandler(axly, m, msg, store)
            } catch (e) {
		...
		try {
			if (isWebpSticker) {
				pathFile = await writeExif(filename, {
					...
					author: options.author || global.db?.set?.[botNumber]?.author || 'axly',
