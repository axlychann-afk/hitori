...
const axlyPath = fileURLToPath(new URL('../axly.js', import.meta.url));

let axlyHandler = null;
const botStartTime = Date.now();
const groupMetadataTimers = {};

/*
	* Create By axly
	* Follow https://github.com/axlydev
	* Whatsapp : https://whatsapp.com/channel/0029VaWOkNm7DAWtkvkJBK43
*/

const reloadHandler = async () => {
	try {
		axlyHandler = (await import(`../axly.js?update=${Date.now()}`)).default;
	} catch (err) {
...
async function GroupUpdate(axly, m, store) {
	...
	if (global.db?.groups?.[m.chat] && store?.groupMetadata?.[m.chat]) {
		...
		if (axly.public && global.db?.groups?.[m.chat]?.setinfo && messages[type]) {
			await axly.sendMessage(m.chat, { text: `${admin} ${messages[type]}`, mentions: [m.sender, ...((normalizedTarget?.id || normalizedTarget)?.includes('@') ? [`${normalizedTarget.id || normalizedTarget}`] : [])].filter(Boolean)}, { ephemeralExpiration: m.expiration || m?.metadata?.ephemeralDuration || store?.messages[m.chat]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 })
		}
		if (type === 20) {
			clearTimeout(groupMetadataTimers[m.chat])
			groupMetadataTimers[m.chat] = setTimeout(async () => {
				store.groupMetadata[m.chat] = await axly.groupMetadata(m.chat).catch(e => ({ ...store.groupMetadata[m.chat] }));
			}, 5000);
			...
			if (m.fromMe && ((jidNormalizedUser(axly.user.id) == (normalizedTarget.id || normalizedTarget)) || (jidNormalizedUser(axly.user.lid) == (normalizedTarget.id || normalizedTarget)))) {
...
async function GroupParticipantsUpdate(axly, update, store) {
	try {
		...
		if (global.db?.groups?.[id] && store?.groupMetadata?.[id]) {
			...
			for (let n of participants) {
				...
				let profile;
				try {
					profile = await axly.profilePictureUrl(jid, 'image');
				} catch {
				...
				if (action === 'add') {
					...
					if (!participant) {
						...
						groupMetadataTimers[id] = setTimeout(async () => {
							store.groupMetadata[id] = await axly.groupMetadata(id).catch(e => ({ ...store.groupMetadata[id] }));
						}, 5000);
					}
				} else if (action === 'remove') {
					if (global.db.groups[id]?.leave) messageText = global.db.groups[id]?.text?.setleave || `@\nLeaving From ${metadata.subject}`;
					if ((jidNormalizedUser(axly.user.lid) == jidNormalizedUser(jid)) || (jidNormalizedUser(axly.user.id) == jidNormalizedUser(jid))) {
				...
				if (messageText && axly.public) {
					await axly.sendMessage(id, {
...
async function LoadDataBase(axly, m) {
	try {
		const botNumber = await axly.decodeJid(axly.user.id);
...
async function MessagesUpsert(axly, message, store) {
	try {
		let botNumber = await axly.decodeJid(axly.user.id);
		...
		if (!store.groupMetadata || Object.keys(store.groupMetadata).length === 0) store.groupMetadata ??= await axly.groupFetchAllParticipating().catch(e => ({}));
		const type = msg.message ? (getContentType(msg.message) || Object.keys(msg.message)[0]) : '';
		const m = await Serialize(axly, msg, store);
		if (axlyHandler) {
			axlyHandler(axly, m, msg, store);
		} else {
			await reloadHandler();
			if (axlyHandler) axlyHandler(axly, m, msg, store);
		}
		if (global.db?.set?.[botNumber]?.readsw && msg.key.remoteJid === 'status@broadcast') {
			await axly.readMessages([msg.key]);
			if (/protocolMessage/i.test(type)) await axly.sendFromOwner(global.db?.set?.[botNumber]?.owner || global.owner, 'Status dari @' + msg.key.participant.split('@')[0] + ' Telah dihapus', msg, { mentions: [msg.key.participant] });
			if (/(audioMessage|imageMessage|videoMessage|extendedTextMessage)/i.test(type)) {
				...
				await axly.sendFromOwner(global.db?.set?.[botNumber]?.owner || global.owner, `Melihat story dari @${msg.key.participant.split('@')[0]}\n${keke}`, msg, { mentions: [msg.key.participant] });
			}
		}
	} catch (e) {
		console.log(message);
...
async function Solving(axly, store) {
	axly.serializeM = (m) => MessagesUpsert(axly, m, store)
	...
	axly.findJidByLid = (lid, store, resolve = false) => {
	...
	axly.getName = (jid, withoutContact  = false) => {
		const id = axly.decodeJid(jid);
		if (id.endsWith('@g.us')) {
			const groupInfo = store.contacts[id] || (store.groupMetadata[id] ? store.groupMetadata[id] : (store.groupMetadata[id] = axly.groupMetadata(id))) || {};
	...
	axly.sendContact = async (jid, kon, quoted = '', opts = {}) => {
		let list = []
		for (let i of kon) {
			list.push({
				displayName: await axly.getName(i + '@s.whatsapp.net'),
				vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await axly.getName(i + '@s.whatsapp.net')}\nFN:${await axly.getName(i + '@s.whatsapp.net')}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Ponsel\nitem2.ADR:;;Indonesia;;;;\nitem2.X-ABLabel:Region\nEND:VCARD`
			})
		}
		axly.sendMessage(jid, { contacts: { displayName: `${list.length} Kontak`, contacts: list }, ...opts }, { quoted, ephemeralExpiration: quoted?.expiration || quoted?.metadata?.ephemeralDuration || store?.messages[jid]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 });
	}
	
	axly.profilePictureUrl = async (jid, type = 'image', timeoutMs) => {
		const result = await axly.query({
			tag: 'iq',
			attrs: {
	...
	axly.relayMessageV2 = async (jid, message, options) => {
		const msg = generateWAMessageFromContent(jid, message, {
			upload: axly.waUploadToServer,
			messageId: generateMessageID(),
			...options
		});
		const hasil = await axly.relayMessage(jid, msg.message, {
	...
	axly.sendPoll = (jid, name = '', values = [], quoted, selectableCount = 1) => {
		return axly.sendMessage(jid, { poll: { name, values, selectableCount }}, { quoted, ephemeralExpiration: quoted?.expiration || quoted?.metadata?.ephemeralDuration || store?.messages[jid]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 })
	}
	
	axly.sendFileUrl = async (jid, url, caption, quoted, options = {}) => {
		...
		try {
			...
			if (mime && mime.includes('gif')) {
				return axly.sendMessage(jid, { video: { url }, caption: caption, gifPlayback: true, ...options }, quotedOptions);
			} else if (mime && mime === 'application/pdf') {
				return axly.sendMessage(jid, { document: { url }, mimetype: 'application/pdf', caption: caption, ...options }, quotedOptions);
			} else if (mime && mime.includes('image')) {
				return axly.sendMessage(jid, { image: { url }, caption: caption, ...options }, quotedOptions);
			} else if (mime && mime.includes('video')) {
				return axly.sendMessage(jid, { video: { url }, caption: caption, mimetype: 'video/mp4', ...options }, quotedOptions);
			} else if (mime && mime.includes('audio')) {
				return axly.sendMessage(jid, { audio: { url }, mimetype: 'audio/mpeg', ...options }, quotedOptions);
			} else {
				return axly.sendMessage(jid, { document: { url }, caption: caption, mimetype: mime, ...options }, quotedOptions);
			}
		} catch (e) {
			return axly.sendMessage(jid, { text: url, ...options }, quotedOptions);
		}
	}
	
	axly.sendGroupInviteV4 = async (jid, participant, inviteCode, inviteExpiration, groupName = 'Unknown Subject', caption = 'Invitation to join my WhatsApp group', jpegThumbnail = null, options = {}) => {
		...
		const invite = await axly.relayMessage(participant, message.message, { messageId: message.key.id })
		return invite
	}
	
	axly.sendFromOwner = async (jids, text, quoted, options = {}) => {
		for (const a of jids) {
			const jid = a.replace(/[^0-9]/g, '') + '@s.whatsapp.net';
			await axly.sendMessage(jid, { text, ...options }, { quoted, ephemeralExpiration: quoted?.expiration || quoted?.metadata?.ephemeralDuration || store?.messages[jid]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 })
		}
	}
	
	axly.sendText = async (jid, text, quoted, options = {}) => axly.sendMessage(jid, { text: text, mentions: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net'), ...options }, { quoted, ephemeralExpiration: quoted?.expiration || quoted?.metadata?.ephemeralDuration || store?.messages[jid]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 })
	
	axly.sendAsSticker = async (jid, pathMedia, quoted, options = {}) => {
		...
		try {
			let anu = await axly.sendMessage(jid, { sticker: { url: result }, ...options }, { quoted, ephemeralExpiration: quoted?.expiration || quoted?.metadata?.ephemeralDuration || store?.messages[jid]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 });
			return anu;
		} finally {
	...
	axly.downloadMediaMessage = async (message) => {
	...
	axly.downloadAndSaveMediaMessage = async (message, filename, attachExtension = true) => {
	    const msg = message.msg || message;
	    msg.mediaKey = fixBytes(msg.mediaKey);
	    msg.fileSha256 = fixBytes(msg.fileSha256);
	    msg.fileEncSha256 = fixBytes(msg.fileEncSha256);
	...
	axly.appendResponseMessage = async (m, text) => {
		let apb = await generateWAMessage(m.chat, { text, mentions: m.mentionedJid }, { userJid: axly.user.id, quoted: m.quoted && m.quoted.fakeObj(), ephemeralExpiration: m.expiration || m?.metadata?.ephemeralDuration || store?.messages[m.chat]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 });
		apb.key = m.key
		apb.key.id = [...Array(32)].map(() => '0123456789ABCDEF'[Math.floor(Math.random() * 16)]).join('');
		apb.key.fromMe = areJidsSameUser(m.sender, axly.user.id);
		if (m.isGroup) apb.participant = m.sender;
		axly.ev.emit('messages.upsert', {
			...m,
	...
	axly.sendMedia = async (jid, pathMedia, fileName = '', caption = '', quoted = '', options = {}) => {
		const { mime, filename, isTemp } = await axly.getFile(pathMedia);
		...
		try {
			if (isWebpSticker) {
				pathFile = await writeExif(filename, {
					...
					author: options.author || global.db?.set?.[botNumber]?.author || 'axlydev',
			...
			let anu = await axly.sendMessage(jid, { [type]: { url: pathFile }, caption, mimetype, fileName, ...options }, { quoted, ephemeralExpiration: quoted?.expiration || quoted?.metadata?.ephemeralDuration || store?.messages[jid]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0, ...options });
			return anu;
		} finally {
	...
	axly.sendAlbumMessage = async (jid, content = {}, options = {}) => {
		...
		await axly.relayMessage(jid, medias.message, { messageId: medias.key.id });
		for (const media of album) {
			const msg = await generateWAMessage(jid, { ...others, ...media }, { upload: axly.waUploadToServer });
			...
			await axly.relayMessage(jid, msg.message, { messageId: msg.key.id });
		}
		return medias;
	}
	
	axly.sendListMsg = async (jid, content = {}, options = {}) => {
		...
		const hasil = await axly.relayMessage(msg.key.remoteJid, msg.message, {
			messageId: msg.key.id,
			additionalNodes: [{
				tag: 'biz',
				attrs: {},
	...
	axly.sendButtonMsg = async (jid, content = {}, options = {}) => {
	...
	axly.newsletterMsg = async (key, content = {}, timeout = 5000) => {
		...
		if (react) {
			...
			const msg = await generateWAMessageContent(media, { upload: axly.waUploadToServer });
			const anu = await axly.query({
				tag: 'message',
	...
	axly.sendCarouselMsg = async (jid, body = '', footer = '', cards = [], options = {}) => {
		async function getImageMsg(url) {
			const { imageMessage } = await generateWAMessageContent({ image: { url } }, { upload: axly.waUploadToServer });
			return imageMessage;
		}
		...
		const hasil = await axly.relayMessage(msg.key.remoteJid, msg.message, { messageId: msg.key.id });
		return hasil
	}
	
	if (axly.user && axly.user.id) {
		const botNumber = axly.decodeJid(axly.user.id);
		if (global.db?.set[botNumber]) {
			axly.public = global.db.set[botNumber].public
		} else axly.public = true
	} else axly.public = true
...
async function Serialize(axly, msg, store) {
	const botLid = axly.decodeJid(axly.user.lid);
	const botNumber = axly.decodeJid(axly.user.id);
	const m = { ...msg };
	if (!m) return m
	if (m.key) {
		...
		if (!m.isGroup && m.chat.endsWith('@lid')) m.chat = axly.findJidByLid(m.chat, store) || m.chat;
		m.sender = axly.decodeJid(m.fromMe && axly.user.id || m.key.participantAlt || m.key.participant || m.chat || '')
		if (m.isGroup) {
			if (!store.groupMetadata) store.groupMetadata = await axly.groupFetchAllParticipating().catch(e => ({}));
			let metadata = store.groupMetadata[m.chat] ? store.groupMetadata[m.chat] : (store.groupMetadata[m.chat] = await axly.groupMetadata(m.chat).catch(e => ({ ...store.groupMetadata[m.chat] })));
			if (!metadata) {
				metadata = await axly.groupMetadata(m.chat).catch(e => ({ ...store.groupMetadata[m.chat] }));
			...
			if (metadata.addressingMode === 'lid') {
				...
				if(!m.sender.endsWith('@g.us')) store.contacts[m.sender] = { ...(store.contacts[m.sender] || {}), id: jidNormalizedUser(m.fromMe && axly.user.lid || participant?.id || store.contacts[m.sender]?.id || m.sender), phoneNumber: jidNormalizedUser(m.fromMe && axly.user.id || participant?.phoneNumber || store.contacts[m.sender]?.phoneNumber || m.sender), name: (m.fromMe && axly.user.name) || m.pushName };
			}
	...
	if (m.message) {
		...
		m.mentionedJid = m.msg?.contextInfo?.mentionedJid?.map(a => axly.findJidByLid(a, store, true)) || []
		...
		if (m.quoted) {
			...
			m.quoted = {
				...
				sender: axly.decodeJid(m.msg.contextInfo.participant),
				fromMe: axly.decodeJid(m.msg.contextInfo.participant) === axly.decodeJid(axly.user.id),
			...
			m.quoted.fromMe = m.quoted.sender === axly.decodeJid(axly.user.id)
			m.quoted.mentionedJid = m.quoted?.msg?.contextInfo?.mentionedJid?.map(a => axly.findJidByLid(a, store, true)) || []
			m.quoted.body = m.quoted.msg?.text || m.quoted.msg?.caption || m.quoted?.message?.conversation || m.quoted.msg?.selectedButtonId || m.quoted.msg?.singleSelectReply?.selectedRowId || m.quoted.msg?.selectedId || m.quoted.msg?.contentText || m.quoted.msg?.selectedDisplayText || m.quoted.msg?.title || m.quoted?.msg?.name || ''
			m.getQuotedObj = async () => {
				if (!m.quoted.id) return null
				let q = await global.loadMessage(m.chat, m.quoted.id, axly)
				if (q) {
					return await Serialize(axly, q, store)
				} else {
					return null
				}
			}
			m.quoted.key = {
				...
				fromMe: areJidsSameUser(axly.decodeJid(m.msg?.contextInfo?.participant), axly.decodeJid(axly?.user?.id)),
				id: m.msg?.contextInfo?.stanzaId
			}
			...
			m.quoted.download = () => axly.downloadMediaMessage(m.quoted)
			m.quoted.delete = () => {
				axly.sendMessage(m.quoted.chat, {
					delete: {
						remoteJid: m.quoted.chat,
	...
	m.download = () => axly.downloadMediaMessage(m)
	
	m.copy = () => Serialize(axly, JSON.parse(JSON.stringify(m)), store)
	
	m.react = (u) => axly.sendMessage(m.chat, { react: { text: u, key: m.key }})
	...
	m.reply = async (content, options = {}) => {
		...
		if (typeof content === 'object') {
			return axly.sendMessage(chat, content, { ...validate, quoted, ephemeralExpiration })
			...
			try {
				if (/^https?:\/\//.test(content)) {
					...
					if (/gif|image|video|audio|pdf|stream/i.test(mime)) {
						...
						return axly.sendMessage(chat, { [type]: { url: content }, caption, mimetype: mime, ...validate }, { quoted, ephemeralExpiration })
					} else {
						return axly.sendMessage(chat, { text: content, mentions: fixMentions, ...validate }, { quoted, ephemeralExpiration })
					}
				} else {
					return axly.sendMessage(chat, { text: content, mentions: fixMentions, ...validate }, { quoted, ephemeralExpiration })
				}
...
// Reload Handler
const watcher = chokidar.watch(axlyPath, {
	ignored: /^\./,
	persistent: true,
	awaitWriteFinish: {
