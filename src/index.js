...
async function startaxlyBot() {
    ...
    global._memoryCleaner = global.safeInterval(
        async () => {
            try {
                // Kuras messageMap total
                if (global.messageMap) {
                    global.messageMap = new Map()
                }
                // Pangkas message store
                ...
                for (const jid of chats) {
                    ...
                    if (Array.isArray(arr) && arr.length > 15) {
                        global.store.messages[jid].array = arr.slice(-15)
                    }
                }
                // Force GC
                if (global.gc) global.gc()
            } catch (e) {
                console.error('[MEMORY CLEANER]', e)
            }
        },
        5 * 60 * 1000
    )
        ...
        async (update) => {
            ...
            try {
                ...
                if (
                    ...
                    setTimeout(() => {
                        console.log(chalk.cyan('[SCHEDULED RESTART] Memory cleanup...'))
                        process.exit(0)
                    }, 6 * 60 * 60 * 1000)
